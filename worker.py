...
class Worker(threading.Thread):
    ...
    def __categories(self):
        #base = self.session.query(db.Product.categories).all()
        categories_id = self.session.query(db.Product).filter_by(deleted=False).all()
        categories = []
        if categories_id == []:
            not_available = "Нет товара"
            not_available_keyboard = [[telegram.InlineKeyboardButton(self.loc.get("menu_cancel"), callback_data=f"{self.loc.get('menu_cancel')}")]]
            self.bot.send_message(self.chat.id, not_available,
                                  reply_markup=telegram.InlineKeyboardMarkup(not_available_keyboard, 
                                  one_time_keyboard=True))
            not_callback = self.__wait_for_inlinekeyboard_callback()
            if not_callback.data == self.loc.get("menu_cancel"):
                self.__user_menu()
        for category in categories_id:
            categories.append(category.categories)
                    
        unique_category = list(set(categories))
        category_names = [unique for unique in unique_category]
        ...
        keyboard = [[telegram.InlineKeyboardButton(category_name, callback_data=f"{category_name}")] for category_name in category_names]
        self.bot.send_message(self.chat.id, self.loc.get("select_category", credit=self.Price(self.user.credit)),
                              reply_markup=telegram.InlineKeyboardMarkup(keyboard, one_time_keyboard=True))

        callback = self.__wait_for_inlinekeyboard_callback()
        choose_category = callback.data.lower()
        if callback.data == self.loc.get("menu_cancel"):
            self.__user_menu()
        else:
            final_category = callback.data.capitalize()
            self.__order_menu(final_category)
    

    def __order_menu(self, final_category):
        """User menu to order products from the shop."""
        log.debug("Displaying __order_menu")
        # Get the products list from the db
        products = self.session.query(db.Product).filter(db.Product.categories == final_category).filter_by(deleted=False).all()
        ...
        while True:
            ...
            # If a Add to Cart button has been pressed...
                ...
                cart[callback.message.message_id][1] += 0.200
    ...
    def __get_cart_value(self, cart):
        ...
        value = self.Price(0)
        for product in cart:
            value += cart[product][0].price * 0.01 * cart[product][1]
        return value
    ...
    def __add_credit_menu(self):
        ...
        # Wait for a reply from the user
        selection = self.__wait_for_specific_message(
            [self.loc.get("menu_credit_card"), self.loc.get("menu_cancel")],
            cancellable=True)
        if selection == self.loc.get("menu_credit_card"):
    ...
    def __edit_product_menu(self, product: Optional[db.Product] = None):
        ...
        # Choose categories
        self.bot.send_message(self.chat.id, self.loc.get("ask_categories"))
        categories = self.__wait_for_regex(r"(.*)").capitalize()
        print(categories)    
        ...
        # If a product is being edited...
            ...
            product.categories = categories if not isinstance(categories, CancelSignal) else product.categories
